...
platform_filters = {
    'LinkedIn': {
        ...
        'scan' : {
            'date'        : lambda x : dt.datetime.strptime(re.search(re.compile('Date: \D{3}, (.+ \+0000 \(UTC\))', flags = re.DOTALL),x).group(1),'%d %b %Y %H:%M:%S %z (%Z)').strftime(CONSTS['date_format']), #standard to gmail format; gets datetime of email sent
...
testing_platform_filters = {
    'LinkedIn': {
        ...
        'eml': {
            ...
            'rejected' : {
                'original_date_applied' : lambda x: dt.datetime.strptime(re.search(re.compile(r"Applied on (\w{3,9} \d{1,3}, \d{4})"),x).group(1),'%B %d, %Y').strftime(CONSTS['date_format']),
        ...
        'email': {
            ...
            'scan' : {
                'date'        : lambda x : lambda x : dt.datetime.strptime(re.sub(r"\s\(\w{3}\)","", x['headers']['Received'].split(';')[1].split(",")[1].strip()), '%d %b %Y %H:%M:%S %z').strftime(CONSTS['date_format']), #standard to gmail format; gets datetime of email sent
